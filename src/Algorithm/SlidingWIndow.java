...
public class SlidingWIndow {
    static ArrayList<Integer> max_of_subarrays(int arr[], int n, int k) {
        ...
        for(int i=0;i<k;i++){
            if(q.peekFirst()!= null && arr[i] < q.peekFirst().getValue()){
                while(!q.isEmpty() && q.peekLast().getValue()< arr[i]){
                    q.pollLast();
                }
                q.addLast(new Pair(i,arr[i]));
            }
            else{
                while(!q.isEmpty() && q.peekFirst().getValue() < arr[i]){
                    q.pollFirst();
                }
                q.addLast(new Pair(i,arr[i]));
            }
        ...
        for(int i=k-1;i<n;i++){
            ...
            while((q.peekFirst()!=null && q.peekFirst().getKey()<=i-k)){
                q.pollFirst();
            }
//            while((q.peekLast()!=null && q.peekLast().getValue()<=arr[i])){
//                q.pollLast();
//            }
            if(q.peekFirst() != null && arr[i] < q.peekFirst().getValue()){
                while((q.peekLast()!=null && q.peekLast().getValue()<=arr[i])){
                ...
                while(q.peekFirst()!=null && q.peekFirst().getValue() <= arr[i]){
                    q.pollFirst();
                };
                q.addLast(new Pair(i,arr[i]));

            }
            result.add(q.peekFirst().getValue());
    ...
    public static void main(String[] args) {
        ...
        System.out.println(SlidingWIndow.max_of_subarrays(arr,183,123));
    }
}
